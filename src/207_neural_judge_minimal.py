...
RESULTS_FILE = SCORES_DIR / "BC_results_minimal.json"

NEURAL_MODEL_FILE = TRAINED_BC_DIR / "neural_classifier_minimal.pth"
NEURAL_SCALER_FILE = TRAINED_BC_DIR / "neural_scaler_minimal.pkl"
...
class VandalismClassifierMinimal(nn.Module):
    """
    Rete neurale per classificazione vandalismo MINIMAL.
...
def load_resources():
    """Carica modello e scaler MINIMAL"""
    print("⏳ Caricamento risorse BC (MINIMAL)...")
    
    if not NEURAL_MODEL_FILE.exists():
        print(f"❌ Modello non trovato: {NEURAL_MODEL_FILE}")
        print("   Esegui prima 17_train_neural_minimal.py")
        return None, None
    ...
    print("   🧠 Caricamento Neural Classifier MINIMAL (PyTorch)...")
    try:
        with open(NEURAL_SCALER_FILE, "rb") as f:
            scaler = pickle.load(f)
        input_dim = scaler.n_features_in_
        model = VandalismClassifierMinimal(input_dim)
        model.load_state_dict(torch.load(NEURAL_MODEL_FILE, map_location='cpu'))
        model.eval()
        print(f"   ✅ Neural Classifier MINIMAL caricato (input_dim={input_dim})")
        return model, scaler
...
def main():
    ...
    print("=" * 60)
    print("🧠 NEURAL JUDGE (MINIMAL) - Real-time Classification")
    ...
    consumer = KafkaConsumer(
        ...
        group_id='bc_judge_minimal_group'
    )
    ...
    try:
        for message in consumer:
            ...
            feat = get_raw_features_minimal(event)
