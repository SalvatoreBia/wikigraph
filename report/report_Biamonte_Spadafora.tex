...
The initial quantitative evaluation was performed on the synthetic dataset generated via \textit{Gemma-3-27B-it}. As shown in Table \ref{tab:synthetic_results}, the \textbf{Neural Complete} model achieved perfect accuracy (100\%), surprisingly matching the \textbf{No RAG} baseline.
...
\vspace{0.2cm}
\noindent \textbf{Case Study: Entity Swapping}
\begin{itemize}
    \item \textbf{Context:} Page \textit{"Gaio Giulio Cesare"}.
    \item \textbf{Edit:} Changed name to \textit{"Gaio Venezia Giulia"} in the infobox.
    \item \textbf{Comment:} "fix nome".
\end{itemize}

In this scenario, the performance hierarchy inverted:
\begin{enumerate}
    \item \textbf{Neural No RAG:} Classified as \textbf{Legitimate}. Lacking access to the Knowledge Graph, the model saw a structurally valid proper noun replacing another and failed to detect the anomaly.
    \item \textbf{LLM Judge (with RAG):} Correctly classified as \textbf{Vandalism}. By retrieving the trusted context via RAG, the LLM successfully reasoned that "Venezia Giulia" is a geographical region, not a Roman dictator, contradicting the ground truth.
\end{enumerate}
...
\textbf{Neural Classifiers} act as a high-speed first line of defense, filtering out 99\% of obvious vandalism (spam, gibberish, and stylistic anomalies). 
However, for the remaining 1\% of subtle, semantic attacks, the \textbf{Graph-RAG LLM Pipeline} is indispensable. It provides the necessary semantic grounding to verify facts, justifying its higher computational cost only on the subset of ambiguous edits flagged by the community detection heuristics.
